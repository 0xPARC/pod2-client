...
pub async fn list_posts(server_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if response.status().is_success() {
        ...
        if let Some(posts_array) = posts.as_array() {
            ...
            println!(
                "{:<6} {:<20} {:<20} {:<8} {:<52} {:<52}",
                "ID", "Created", "Last Edited", "Docs", "Latest Main Pod", "Latest Timestamp Pod"
            );
            println!("{}", "-".repeat(160));
...
fn print_post_row(post: &serde_json::Value) {
    ...
    let last_edited_at = post
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("N/A");
    let documents = post.get("documents").and_then(|d| d.as_array());
    ...
    let (latest_main_pod, latest_timestamp_pod) = if let Some(docs) = documents {
        if let Some(latest_doc) = docs
            .iter()
            .max_by_key(|doc| doc.get("revision").and_then(|v| v.as_i64()).unwrap_or(0))
        {
    ...
    println!(
        ...
        last_edited_at,
        document_count,
        truncate_pod_json(&latest_main_pod),
        truncate_pod_json(&latest_timestamp_pod)
    );
}
