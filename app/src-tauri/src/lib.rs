...
use tauri::{App, AppHandle, Emitter, EventLoopMessage, Manager, State, Wry};
...
async fn send_message_as_pod(
    ...
    let mut builder = pod2::frontend::SignedPodBuilder::new(&params);
    ...
    let mut signer = pod2::backends::plonky2::signedpod::Signer(private_key);
    ...
    let serialized_signed_pod: pod2::frontend::SerializedSignedPod = signed_pod.into();
